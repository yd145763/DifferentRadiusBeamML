# -*- coding: utf-8 -*-
"""
Created on Wed Mar 20 21:43:23 2024
...
file_path = "C:\\Users\\ADMIN\\Downloads\\DNNPredictBeamDataGPU_4TrainingRadius\\"
file_path2 = "C:\\Users\\ADMIN\\Downloads\\"
file_path3 = "C:\\Users\\ADMIN\\Downloads\\3dplot\\"
steps = 20
...
if Plot_all_i == True:
    ...
    ape_i = []
    for i, i_z in zip(I[::steps], I_z[::steps]): 
        ...
        ape_n = []
        N_x = np.linspace(-25, 25 ,df_df.shape[1])
        
        for n, n_x in zip(N[::steps], N_x[::steps]):
            ...
            plt.ylabel("E-Field (eV)")
            plt.legend(["Actual", "Prediction"], prop={'weight': 'bold','size': 10}, loc = "upper left")
            plt.title("Actual/Prediction\n"+"n = "+str(n)+", x = "+str(round(n_x,1))+"µm"+"\n"+"i = "+str(i)+", z = "+str(round(i_z,1))+"µm"+"\n\n", fontweight = 'bold')
            plt.show()
            plt.close()
            
            df_predicted_contour[n] = e_predicted
            df_actual_contour[n] = e_actual
        x = np.linspace(-25, 25, num=df_actual_contour.shape[1])
        ...
        ax.xaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        ax.yaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        plt.title("Actual Beam (i = "+str(i)+", z = "+str(round(i_z,1))+"µm"+")\n", fontweight = 'bold', fontsize=15)
        plt.show()
        plt.savefig(file_path3+'actual_beam_'+str(i)+'.jpg', format='jpg')
        plt.close()
        ...
        ax.xaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        ax.yaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        plt.title("Predicted Beam (i = "+str(i)+", z = "+str(round(i_z,1))+"µm"+")\n", fontweight = 'bold', fontsize=15)
        plt.show()
        plt.savefig(file_path3+'predicted_beam_'+str(i)+'.jpg', format='jpg')
        plt.close()
...
if Plot_selected_i == True:
    ...
    for i, i_z in zip(I[::steps], I_z[::steps]): 
        ...
        ax.xaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        ax.yaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))
        plt.title("Generated Beam (i = "+str(i)+", z = "+str(round(i_z,1))+"µm"+")\n", fontweight = 'bold', fontsize=15)
        plt.show()
        plt.savefig(file_path3+'predicted_beam_'+str(i)+'.jpg', format='jpg')
        plt.close()
