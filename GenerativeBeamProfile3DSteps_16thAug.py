...
for dense_layer in dense_layers:
        for layer_size in layer_sizes:
            ...
            history = model.fit(X_train, y_train, epochs=100,validation_data=(X_test, y_test), batch_size = 500
            ...
            mae_n_mean_i_mean = []
            for i in I[::training_steps]: 
                df_predicted_i_filtered = df_predicted[df_predicted['i'] == i] #predicted
                df_main_main = df_main_main_full[df_main_main_full['i'] ==i] #actual
                ...
                for n in N[::training_steps]:
                    ...
                    ax.plot(df_predicted_i_n_filtered['y'], e_predict, color = "blue") 
                    #graph formatting     
                    ax.tick_params(which='major', width=2.00)
                    ax.tick_params(which='minor', width=2.00)
                    ax.xaxis.label.set_fontsize(15)
                    ...
                    error = e_actual - e_predict
                    percentage_error = (error.abs() / e_actual) * 100
                    ape = percentage_error.mean()
                    ape_n.append(ape)
                    
                    mae = mean_absolute_error(e_actual, e_predict)
                    mae_n.append(mae)
...
min_index = df_results['ape_list'].idxmin()
layer_size_op = df_results.iloc[min_index, 0]
dense_layer_op = df_results.iloc[min_index, 1]
steps = 5
training_steps = 5
...
for i in I[::training_steps]: 
    ...
    plt.show()
    plt.close()
    df_predicted_contour.to_csv(file_path+"optimized3dpredictedmultibeam_model_nodes"+str(layer_size_op)+"_layers"+str(dense_layer_op)+"_i"+str(i)+".csv")
    df_actual_contour.to_csv(file_path+"optimized3dactualmultibeam_model_nodes"+str(layer_size_op)+"_layers"+str(dense_layer_op)+"_i"+str(i)+".csv")
...
model.save(file_path+"optimized3dmultisinglebeam_model_nodes"+str(layer_size_op)+"_layers"+str(dense_layer_op))
df_training_validation.to_csv(file_path+"optimizedtraining_validation_result.csv")